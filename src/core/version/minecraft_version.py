...
Example:
    获取最新的正式版本::
        ...
        import asyncio
        from src.core.version.minecraft_version import get_versions
...
from src.core.Repository import config, persistence
from src.utils.request import request_json
...
class VersionRequestOfficial:
    ...
    async def request(
        ...
        error: Optional[Callable[[str], None]] = empty,
        ...
        return await request_json(
            self.host + self.path, finnished=finnished, error=error
        )
...
async def load_versions(
    ...
    result: Union[Dict[str, Any], List[Any], None] = await API.request(
        finnished=finnished, error=error
    )
...
async def get_versions(
    ...
    # 检查缓存，过期时间 600 秒
    if not persistence["minecraft.version_manifest"] or (
        time.time() - persistence.get("minecraft.version_manifest.update_time", 0) > 600
    ):
